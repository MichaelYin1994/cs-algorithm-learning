...
def Model(_j, _abnormal_data, _abnormal_label, _hidden_num, _elem_num, _file_name, _partition):
    ...
    with g.as_default():
        ...
        if cell_type == 1:
            ...
            enc_cell = RSLSTMCell(_hidden_num, file_name=_file_name, component=_j, partition=_partition, type='enc')
            dec_cell = RSLSTMCell(_hidden_num, file_name=_file_name, component=_j, partition=_partition, type='dec')
        if cell_type == 2:
        ...
        with tf.variable_scope("encoder"):
            if _j == 0:
                ...
                enc_outputs = []
                for step in range(len(p_inputs)):
                    enc_output_, enc_state = residual_enc_cell(p_inputs[step], enc_state)
                    enc_outputs.append(enc_output_)
            else:
                enc_state = enc_cell.zero_state(batch_size=batch_num, dtype=tf.float32)
                enc_outputs = []
                for step in range(len(p_inputs)):
                    enc_output_, enc_state = enc_cell(p_inputs[step], enc_state)
                    enc_outputs.append(enc_output_)


        # with tf.device('/device:GPU:1'):
        with tf.variable_scope('decoder') as vs:
            dec_weight_ = tf.Variable(tf.truncated_normal([_hidden_num, _elem_num], dtype=tf.float32), name="dec_weight")
                                ...
                                if partition:
                                    ...
                                    for i in range(k_partition):
                                        error_partition, precision_partition, recall_partition, f1_partition, roc_auc_partition, pr_auc_partition, cks = RunModel(
                                            splitted_data[i], splitted_label[i], hidden_num, elem_num, _file_name=file_name, _partition=i)
                                        final_error.append(error_partition)
                                    # print('-----------------------------------------')
                                    final_error = np.concatenate(final_error).ravel()
                                    final_zscore = Z_Score(final_error)
                        ...
                        for _, _, files in os.walk(root + '/' + dir):
                            for file in files:
                                ...
                                if partition:
                                    ...
                                    for i in range(k_partition):
                                        error_partition, precision_partition, recall_partition, f1_partition, roc_auc_partition, pr_auc_partition, cks = RunModel(
                                            splitted_data[i], splitted_label[i], hidden_num, elem_num, _file_name=os.path.splitext(os.path.basename(file_name))[0], _partition=i)
                                        final_error.append(error_partition)
                                    # print('-----------------------------------------')
                                    final_error = np.concatenate(final_error).ravel()
                                    final_zscore = Z_Score(final_error)
