...
def OCSVMModel(_abnormal_data, _kernel='rbf', _nu=0.5):
    clf = svm.OneClassSVM(kernel=_kernel, nu=_nu)
...
def RunModel(_file_name, _choice, _kernel='rbf', _nu=0.5):
    if _choice == 1:
    ...
    np.savetxt('./saved_result/' + pathlib.Path(_file_name).parts[0] + '/oc-svm_' + os.path.basename(_file_name) + '_score.txt', score_pred_label, delimiter=',')
    ...
    # for k in k_number:
    #     precision_at_k = CalculatePrecisionAtK(abnormal_label, score, k, _type=2)
