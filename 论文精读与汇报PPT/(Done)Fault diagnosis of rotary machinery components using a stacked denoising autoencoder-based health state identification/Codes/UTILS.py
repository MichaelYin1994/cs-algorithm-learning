...
def generating_cutted_sequences(data=[], window=20, stride=10, portion=0.4, verboseRound=500):
    ...
    for ind, item in enumerate(data):
        ...
        cuttedSeqs.append(sequence_to_subsequence(data[ind], window, stride, portion))
    print("End time : {}".format(datetime.now()))
    print("=====================================================")
    return cuttedSeqs

def sequence_to_subsequence_featurepoint(seq=[], window=20, stride=10, portion=0.4):
    pass
...
def sequence_to_subsequence(seq=[], window=20, stride=10, portion=0.4):
    ...
    @Return:
    ----------
    dict-like, cuttting results. It acts like:
        {0: [[cutted seq], [mean], [var]], ...}
